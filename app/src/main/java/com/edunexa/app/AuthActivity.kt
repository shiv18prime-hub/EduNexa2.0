...
class AuthActivity : AppCompatActivity() {
                ...
                if (googleLoginMode) {
                    db.collection("users").document(user.uid).get().addOnSuccessListener { doc ->
                        if (doc.exists() && doc.getString("role") != null) {
                            ...
                            status.text = "No EduNexa account is linked to this Google account. Create an account first."
                        }
                    }.addOnFailureListener {
                        progress.visibility = View.GONE
                        status.text = it.message ?: "Could not check your EduNexa account"
                    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val header = findViewById<ImageView>(R.id.formHeaderImage)
        ...
        if (firebaseAuth.currentUser != null) {
            welcome.visibility = View.GONE; form.visibility = View.GONE; progress.visibility = View.VISIBLE
            status.text = "Restoring your session..."; routeCurrentUser(status, progress)
        }
        ...
        fun signup(role: String) {
            reset(); selectedRole = role; googleLoginMode = false; welcome.visibility = View.GONE; form.visibility = View.VISIBLE
            register.visibility = View.VISIBLE; login.visibility = View.GONE; social.visibility = View.VISIBLE
            val isSchool = role == "school"; header.setImageResource(if (isSchool) R.drawable.school_art else R.drawable.student_art)
            title.text = if (isSchool) "Create School Account" else "Create Student Account"
            subtitle.text = if (isSchool) "Register your school to manage students and academic activities" else "Start your learning journey with EduNexa"
            name.visibility = if (isSchool) View.GONE else View.VISIBLE; school.visibility = if (isSchool) View.VISIBLE else View.GONE
            classInput.visibility = if (isSchool) View.GONE else View.VISIBLE; contact.visibility = if (isSchool) View.VISIBLE else View.GONE
            address.visibility = if (isSchool) View.VISIBLE else View.GONE; approval.visibility = if (isSchool) View.VISIBLE else View.GONE
        }
        fun showLogin() {
            reset(); googleLoginMode = true; welcome.visibility = View.GONE; form.visibility = View.VISIBLE
            header.setImageResource(R.drawable.edunexa_logo); title.text = "Welcome Back"; subtitle.text = "Login with Email or Google"
            listOf(name, school, classInput, contact, address, register, approval).forEach { it.visibility = View.GONE }
            login.visibility = View.VISIBLE; social.visibility = View.VISIBLE
            findViewById<View>(R.id.emailContinueBtn).visibility = View.GONE
        }
        fun showWelcome() { reset(); googleLoginMode = false; progress.visibility = View.GONE; form.visibility = View.GONE; welcome.visibility = View.VISIBLE }

        findViewById<View>(R.id.studentChoiceBtn).setOnClickListener { signup("student") }
        findViewById<View>(R.id.schoolChoiceBtn).setOnClickListener { signup("school") }
        findViewById<View>(R.id.showLoginBtn).setOnClickListener { showLogin() }
        findViewById<View>(R.id.loginLink).setOnClickListener { showLogin() }
        ...
        findViewById<View>(R.id.googleBtn).setOnClickListener {
            ...
            status.text = "Opening Google account chooser..."
        ...
        login.setOnClickListener {
            ...
            progress.visibility = View.VISIBLE
            authRepository.login(email.text.toString().trim(), password.text.toString()) { ok, msg -> status.text = msg; if (ok) routeCurrentUser(status, progress) else progress.visibility = View.GONE }
        }
    }

    override fun onStart() {
        super.onStart()
        if (::status.isInitialized && firebaseAuth.currentUser != null && !routingSession) routeCurrentUser(status, progress)
    }

    private fun routeCurrentUser(status: TextView, progress: ProgressBar) {
        if (routingSession) return
        val uid = firebaseAuth.currentUser?.uid ?: run { progress.visibility = View.GONE; return }
        routingSession = true; progress.visibility = View.VISIBLE
        db.collection("users").document(uid).get().addOnSuccessListener { doc ->
            routingSession = false; progress.visibility = View.GONE
            when (doc.getString("role")) {
                ...
                else -> { status.text = "Account role is not configured"; firebaseAuth.signOut() }
            }
        }.addOnFailureListener {
            routingSession = false; progress.visibility = View.GONE
